...
```
Total = **22 variants per entry** (5+5+1+5+3+3).
...
1. **Backbone first.** Decide ONCE per entry and reuse across all 6 formats: the core reframe, the proof point (real name/number from the call), a signature payoff line, and ONE uppercase **CTA code word**.
...
3. **Bodies use literal `\n` line breaks between beats** — one beat per line, e.g. `"[SETUP] …\n[TENSION] …\n[PAYOFF] …"` (simpler variants use `[VIBE]/[ONE LINE]/[CAPTION CARRIES IT]`, `[1]/[2]/[3]`, `[ONE SHOT]/[TEXT TURN]/[BUTTON]`, etc.).
4. **Stage directions (camera, length in seconds, music, pacing) go ONLY in `notes`** — never in hook/body/cta.
5. **CTA code word is identical across all 6 formats** (phrasing adapts: Comment/DM/Save/Tag/Follow). A few Tag/Save/Follow CTAs may stand alone, mirroring the gold standard's distribution.
